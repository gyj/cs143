...
class Mininet( object ):
    ...
    def addHost( self, name, mac=None, ip=None ):
        ...
        if name in self.nameToNode:
            error( 'name already in network: %s\n' % name )
            assert False, ( 'name already in network: %s\n' % name )
            pass
    ...
    def buildFromTopo( self, topo ):
        ...
        def addNode( prefix, addMethod, nodeId , name):
        ...
        for hostId in sorted( topo.hosts() ):
            addNode( 'h', self.addHost, hostId, topo.get_node_name(hostId) )
        info( '\n*** Adding switches:\n' )
        for switchId in sorted( topo.switches() ):
            addNode( 's', self.addSwitch, switchId, topo.get_node_name(switchId) )
        info( '\n*** Adding links:\n' )
    ...
    # def addLink(self, node1, node2, delay=None):
    #     """
    #     delay, if not None, should be an integer, in millisecond
    #     """
    #     assert self.topo is None, 'currently only supports addLink() when the net was not created with any topology'
    #     if delay:
    #         assert self.built
    #         pass
    #     # src and dst are host objects
    #     if type(node1) is str:
    #         node1 = self.nameToNode[node1]
    #         pass
    #     else:
    #         # this is slow (O(n)) since self.hosts is a list
    #         assert node1 in self.hosts or node1 in self.switches
    #         pass
    #     if type(node2) is str:
    #         node2 = self.nameToNode[node2]
    #         pass
    #     else:
    #         assert node2 in self.hosts or node2 in self.switches
    #         pass
    #     createLink(node1, node2)
    #     if not delay is None:
    #         self.configLinkDelay(node1.name, node2.name, delay)
    #         pass
    #     return

    def _setLinkDelays(self):
        for srcId, dstId in sorted( self.topo.edges() ):
            src, dst = self.idToNode[ srcId ], self.idToNode[ dstId ]
            self._configLinkDelay(
                src, dst, self.topo.edge_delay[(srcId, dstId)])
            self._configLinkDelay(
                dst, src, self.topo.edge_delay[(dstId, srcId)])
            info( '(%s, %s) ' % ( src.name, dst.name ) )
            pass
        pass
    ...
    def _configLinkDelay( self, src, dst, delay ):
        """Change status of src <-> dst links.
           src: node object
           dst: node object
           delay: integer for delay in millesecond. both directions use this delay"""
        if delay is None:
            return
        if src.name not in self.nameToNode:
            error( 'src not in network: %s\n' % src.name )
        elif dst.name not in self.nameToNode:
            error( 'dst not in network: %s\n' % dst.name )
        else:
            srcNode, dstNode = src, dst
            connections = srcNode.connectionsTo( dstNode )
            if len( connections ) == 0:
                error( 'src and dst not connected: %s %s\n' % ( src, dst) )
            for srcIntf, dstIntf in connections:
                result = srcNode.cmd( 'tc qdisc add dev %s root netem delay %ums' %( srcIntf, delay))
                if result:
                    error( 'link src status change failed: %s\n' % result )
                result = dstNode.cmd( 'tc qdisc add dev %s root netem delay %ums' %( dstIntf, delay))
                if result:
                    error( 'link dst status change failed: %s\n' % result )
