...
class Topo(object):
    ...
    def add_node(self, dpid, node, name=None):
        '''Add Node to graph.
        ...
        self.idToName[dpid] = name

    def add_edge(self, src, dst, edge = None, delay = None):
        '''Add edge (Node, Node) to graph.

        @param src src dpid
        @param dst dst dpid
        @param delay integer value, in millisecond. both directions will use this delay.
        ...
        self.edge_delay[(src, dst)] = delay
        self.edge_delay[(dst, src)] = delay
    ...
    def get_node_name(self, dpid):
        return self.idToName.get(dpid, None)
